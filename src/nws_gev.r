...
# There is also a problem with the apparent completeness of the dataset
# with earlier and latest events being too sparse. A straightforward check
# using Kolmogorov-Smirnov against stationary (Poisson) rate yields the
# acceptable cut-off points, leaving a total of 485 events.
events <- events[95:566,]
...
# First, set up the magnitude steps. For NWS events, between 2.0 and 5.5
GR_M_Range <- seq(2.0, 5.4, by=0.1)
...
#pdf(file="nws_gev.pdf",paper="a4",width=0,height=0,pointsize=10)
op <- par(mfrow = c(3,2))
...
matplot(Time_Steps,Quantiles,ylim=c(6,8),type="l",lty=1,col=1,main="GEV Maximum Magnitude Estimation",xlab="T Window (days)", ylab="0.975 Magnitude Quantile");grid(lty=2,col=5)
par(op)
#dev.off()